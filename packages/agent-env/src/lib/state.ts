 ...
 *
 * Excludes:
 * - `.agent-env/` — state files and baseline config
 * - `.vscode/statusBar.json` — generated by purpose command from template
 *
 ...
 * - Cloned repos won't show generated files as dirty
 *
 * Idempotent: does nothing if patterns are already present.
 *
...
export async function ensureGitExclude(
  ...
  try {
    ...
    // Find patterns not yet in the exclude file
    const missing = GIT_EXCLUDE_PATTERNS.filter(
      (pattern) => !lines.some((line) => line.trim() === pattern)
    );

    if (missing.length === 0) {
      return; // All patterns already excluded
    }

    // Append missing patterns with newline prefix if file doesn't end with newline
    const prefix = content.endsWith('\n') ? '' : '\n';
    const toAppend = missing.map((p) => `${p}\n`).join('');
    await deps.appendFile(excludePath, `${prefix}${toAppend}`, 'utf-8');
  } catch (err) {
